...
const BASE = { narrative: 76, tone: 76, emotion: 82, message: 68 };

function jitter(base: number): number {
  const delta = Math.round((Math.random() - 0.5) * 16);
  return Math.min(97, Math.max(40, base + delta));
}

export function scoreConsistency(): ConsistencyReport {
  return {
    narrative: jitter(BASE.narrative),
    tone: jitter(BASE.tone),
    emotion: jitter(BASE.emotion),
    message: jitter(BASE.message),
